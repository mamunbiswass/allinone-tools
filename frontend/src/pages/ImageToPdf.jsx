...
import { Image, Download, Trash2, FileImage, UploadCloud, Scale } from "lucide-react";

export default function ImageToPdf() {
  const [images, setImages] = useState([]);
  const [loading, setLoading] = useState(false);
  const [isDragging, setIsDragging] = useState(false);
  const [fitMode, setFitMode] = useState("actual"); // "actual" | "fit"
  const dropRef = useRef(null);

  // 📂 Handle Image Upload (manual or drag-drop)
    ...
    if (fitMode === "actual") {
      ...
      images.forEach((img, index) => {
        ...
        if (imgAspect > pageAspect) {
          renderWidth = pageWidth - 20; // margin
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center bg-gradient-to-br from-blue-50 via-cyan-50 to-green-100 p-4">
      <div className="bg-white shadow-2xl rounded-2xl p-8 w-full max-w-lg transform hover:scale-[1.02] transition-all">
        ...
        >
          ...
          <p className="text-gray-600 text-sm text-center mb-2">
            {isDragging
              ? "Drop images here..."
              : "Drag & drop your images or click below"}
          </p>
